...
# Why this exists:
#
# oslo_service.ProcessLauncher forks one child process per backend. On
# SIGTERM, oslo_service's child SIGTERM handler calls SignalHandler.clear()
# which resets all signal handlers to SIG_DFL. The parent ProcessLauncher
# then sends a second SIGTERM to each child via os.kill(child_pid, SIGTERM).
# With the handler now SIG_DFL, the child terminates immediately — even
# though it's still inside pool.waitall() waiting for an in-flight RPC
# handler to finish.
#
...
class Service(service.Service):
    ...
    def _drain_pool(self, server_name, server, timeout) -> None:
        ...
        try:
            ...
            # pool.running() may not be numeric (e.g., in unit tests with
            # mocked rpcservers). Skip drain entirely.
            return
    ...
    def _phase3_teardown(self) -> None:
        ...
        conn.stop_consuming() — same eventlet socket race we avoided in
        Phase 1. rpcserver.wait() hangs in _work_executor.shutdown(wait=True)
        because the GreenPool we already drained in Phase 2 is the same
        pool the work executor manages.

        Instead we let the process exit naturally after this method
        returns. The OS closes the AMQP socket as the interpreter shuts
        down; RabbitMQ detects the broken connection and requeues any
        pending messages to a healthy instance.
        """
        ...
        # Reset draining flag on the manager now that shutdown is complete.
        # In production this is a no-op (process exits immediately after).
        # In tests this prevents stale _draining=True from leaking into
        # subsequent test methods that reuse the same fake RPC endpoint.
        if hasattr(self.manager, '_draining'):
